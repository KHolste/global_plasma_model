...
Die ordnugsgemäße Installtion kann mit Test.py überprüft werden. Dazu wird der Speicherort dieser Datei benötigt. 
...
Sollten sich alle sechs Python Dokumente:	KryptonFlussVariation.py
											KryptonPerformance.py
											KryptonPRFVariation.py
											IodFlussVariation.py
											IodPerformance.py
											IodPRFVariation.py
und die Modelloberfläche in einem Ordner befinden, kann die Labviewoberfläche gestartet werden.

Bedienung
Durch einen Schieberegler kann der Treibstoff ausgewählt (Iod oder Krypton). Des weiteren müssen grundlegende 
Betriebsparameter eingetragen werden. Die Beta-Faktoren reduzieren die geometrische Fläche der Triebwerksfläche.
Die Buchstaben bezeichnen dabei Ionen (i) und Neutralgas (g).
...
Sollstrom angegeben werden. In das Textfeld müssen die zu berechnenden Teilchenflüsse in sccm eingegeben werden. Dabei werden
...
die Status der Berechnungen an. Das Fenster darf minimiert, aber nicht geschlossen werden. Nach Beendigung der Berechnung
...
Das Labview VI Hilfe von P. Dietz kann dazu genutzt werden eine linear oder exponentiell ansteigende Folge von Zahlen zu
